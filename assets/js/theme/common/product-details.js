...
export default class ProductDetails extends ProductDetailsBase {
    ...
    productOptionsChanged(event) {
        ...
        utils.api.productAttributes.optionChange(productId, $form.serialize(), 'products/bulk-discount-rates', (err, response) => {
            ...
            customJS.getReviewOnOptionChanged(productId,this.$scope);
    ...
    showProductImage(image) {
        if (isPlainObject(image)) {
            ...
            const originalMainImg = $('.slider-single .slick-current img').attr('src').split('?')[0];
            const originalZoomImg = $('.slider-single .slick-current').attr('href');

            if (!document.querySelector('.slider-single .slick-current img').hasAttribute("data-swatch-src")) {
                document.querySelector('.slider-single .slick-current img').setAttribute('data-swatch-src', originalMainImg);
                document.querySelector('.slider-single .slick-current').setAttribute('data-swatch-zoom-pathurl', originalZoomImg);
            }

            $('.slider-single .slick-current img').attr('src', mainImageUrl);
            $('.slider-single .slick-current').attr('href', zoomImageUrl);
    ...
    customScripts() {
        $(".slider-single").not('.slider-single.slick-initialized').slick({
            slidesToShow: 1,
            slidesToScroll: 1,
            arrows: false,
            asNavFor: ".slider-nav",
        ...
        $(".slider-nav").not('.slider-nav.slick-initialized').slick({
            infinite: true,
            dots: false,
            slidesToShow: 4,
            slidesToScroll: 1,
